...
import           Language.Fortran.Annotations       (A, unitAnnotation)
import           Language.Fortran.Helpers
...
--parseSrcString:: Maybe FortranVersion -> FM.ModFiles -> String -> IO (Maybe (F.ProgramFile A, SourceText))
parseSrcString mv mods s = do
  let inp = flexReadString s
  let result = case mv of
        Nothing -> FP.fortranParserWithModFiles mods inp s
        Just v  -> FP.fortranParserWithModFilesAndVersion v mods inp s
  case result of
  ...
  where
    ...
    flexReadString :: String -> B.ByteString
    flexReadString = (encodeUtf8 . decodeUtf8With (replace ' ')) . B8.pack
