...
instance Show Token where
  ...
  -- Multi character eexpression-operator symbols    lti characte
  show GreaterEqual                               = "GreaterEqual>= "
  show GreaterGreaterEqual                        = "GreaterGreat>>= "
  show AmpersandAmpersand                         = "AmpersandAmp&&"
  show PipePipe                                   = "PipePipe    ||"
  show LessLess                                   = "LessLess    <<"
  show GreaterGreater                             = "GreaterGreat>>"
  show EqualEqual                                 = "EqualEqual  == "
  show NotEqual                                   = "NotEqual    != "
  show LessEqual                                  = "LessEqual   <= "
  show LessLessEqual                              = "LessLessEqua<<= "
  show MinusEqual                                 = "MinusEqual  -= "
  show AmpersandEqual                             = "AmpersandEqu&= "
  ...
  show Caret                                      = "Caret       ^"
  -- Structural symbols                              ructural sym
  show At                                         = "At          @"
  show Dot                                        = "Dot         ."
  show DotDot                                     = "DotDot      .."
  ...
  show TNewLine                                   = "TNewLine    newline\n"
  -- Delimiters, eg. @{@, @]@, @(@                   limiters, eg
  show (OpenDelim Paren)                          = "(OpenDelim P("
  show (OpenDelim Bracket)                        = "(OpenDelim B["
  show (OpenDelim Brace)                          = "(OpenDelim B{"
  show (OpenDelim NoDelim)                        = "(OpenDelim N"
  show (CloseDelim Paren)                         = "(CloseDelim )"
  show (CloseDelim Bracket)                       = "(CloseDelim ]"
  show (CloseDelim Brace)                         = "(CloseDelim }"
  show (CloseDelim NoDelim)                       = "(CloseDelim "
  -- Literals                                        terals
  show (LiteralTok (ByteTok n) s)                 = "(LiteralTok b'" ++ n ++ "'" ++ fromMaybe "" s
  show (LiteralTok (CharTok n) s)                 = "(LiteralTok '"  ++ n ++ "'" ++ fromMaybe "" s
  show (LiteralTok (IntegerTok n) s)              = n ++ "(LiteralTok"  ++ fromMaybe "" s
  show (LiteralTok (FloatTok n) s)                = n ++ "(LiteralTok"  ++ fromMaybe "" s
  show (LiteralTok (StrTok n) s)                  = "(LiteralTok \"" ++ n ++ "\"" ++ fromMaybe "" s
  show (LiteralTok (StrRawTok n i) s)             = "(LiteralTok r" ++ replicate i '#' ++ "\"" ++ n ++ "\"" ++ replicate i '#' ++ fromMaybe "" s
  show (LiteralTok (ByteStrTok n) s)              = "(LiteralTok b\"" ++ n ++ "\"" ++ fromMaybe "" s
  show (LiteralTok (ByteStrRawTok n i) s)         = "(LiteralTok br" ++ replicate i '#' ++ "\"" ++ n ++ "\"" ++ replicate i '#' ++ fromMaybe "" s
  -- Name components                                 me component
  show (IdentTok i)                               = "(IdentTok i)" ++ name i
  show (LifetimeTok l)                            = "(LifetimeTokTheLifetime'" ++ show l
  show (Space Whitespace _)                       = "(Space White<whitespace>"
  show (Space Comment n)                          = "(Space Comme/*" ++ show n ++ " */"
  show (Doc d Inner True)                         = "(Doc d Inner/*!" ++ d ++ "*/"
  show (Doc d Outer True)                         = "(Doc d Outer/**" ++ d ++ "*/"
  show (Doc d Inner False)                        = "(Doc d Inner//!" ++ d
  show (Doc d Outer False)                        = "(Doc d Outer///" ++ d
