...
processQQ safety isPure (QQParseR rustRet rustBody rustNamedArgs) = do
  ...
  (haskArgs, reprCArgs) <- unzip <$> traverse (getAType . void) rustArgs
...
processQF safety isPure (QQParseF rustRet rustBody rustNamedArgs) = do
  ...
  debugIt "====rustRet : " [void rustRet]
{-
  aa@(haskRet, reprCRet) <- case rustRet of
                              Nothing -> undefined
                              Just r -> getAType (void r)
...
processFQ safety isPure (QQParseR rustRet rustBody rustNamedArgs)
                        (FQParse fortRet fortBody fortNamedArgs) = do
